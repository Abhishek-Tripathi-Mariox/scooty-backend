...
module.exports = {
  ...
  transactions: async (req, res, next) => {
    const data = await StationAdminService().listTransactions({
      stationAdminId: req.body.stationAdminId,
      stationId: req.query.stationId,
      type: req.query.type,
      from: req.query.from,
  ...
  bookingInvoice: async (req, res, next) => {
    const invoice = await StationAdminService().bookingInvoice({
      stationAdminId: req.body.stationAdminId,
      stationId: req.query.stationId,
      bookingId: req.params.bookingId,
    });
  ...
  bookingRefund: async (req, res, next) => {
    try {
      const booking = await StationAdminService().updateBookingRefund({
        stationAdminId: req.body.stationAdminId,
        stationId: req.body.stationId || req.query.stationId,
        bookingId: req.params.bookingId,
        status: req.body.status,
